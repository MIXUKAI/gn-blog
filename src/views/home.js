...
const LoadingRoute = ({ component: Component, loading, ...rest }) => (
        ...
        <div style={{ flexBasis: '75%', position: 'relative' }}>
          <Switch>
            <Route 
              exact path="/"
              render={(props) => (
                <Redirect to="/page/1" {...props}/>
              )}
            />
            <LoadingRoute path="/page/:page" loading={this.handleLoding} component={Article}/>
            <LoadingRoute exact path="/tags" loading={this.handleLoding} component={Archive}/>
            <LoadingRoute path="/tags/:name" loading={this.handleLoding} component={Archive}/>
          </Switch>
        </div>
      </Layout>
    );
